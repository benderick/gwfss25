...
基础网络由 BEiTv2-Large、ViT-Adapter 和 Mask2Former 构成 \citep{peng2022beitv2,chen2023vitadapter,cheng2022mask2former}。编码器采用 ImageNet-1K 预训练权重，像素解码路径使用双线性上采样，输入裁剪尺寸为 $512\times512$。AdamW 优化器的解码器初始学习率为 $1\times10^{-4}$，编码器学习率乘子为 0.1，权重衰减为 0.05。监督预热和半监督联合训练分别执行 20,000 和 90,000 次迭代；受显存限制，联合训练采用全局批大小 1，即每次迭代分别读取 1 张有标注图像和 1 张无标注图像，所有自有方法与消融均保持该设置。弱增强包括多尺度短边缩放、$512\times512$ 随机裁剪、SSD 颜色扰动和随机水平翻转，学生分支额外采用颜色抖动、随机灰度和高斯模糊。监督阶段选出的同构最优参数被显式加载到教师和学生，不混用带 SAPA 上采样器的项目 checkpoint，也不重新初始化分割头；EMA 动量设为 0.9996，并从联合训练的第一次迭代起连续更新，训练期间不再执行冻结后的教师硬替换。

每 5,000 次迭代以分块方式计算一次全模型师生参数 RMS 距离，用于诊断 EMA 轨迹而不参与优化。

\TRPL 使用原尺度与 0.75 倍尺度两个教师视图，背景、麦穗、茎秆和叶片的置信阈值依次为 0.70、0.70、0.55 和 0.70。不确定性权重 $\beta$ 和温度 $\tau_u$ 分别为 1.0 和 0.5，最大可接受不确定性为 0.75；骨架阈值、持续性阈值和边界半径分别为 0.35、0.5 和 2 像素。区域 Dice、拓扑和骨架一致性损失的权重依次为 0.5、0.5 和 0.25。\TCPM 为 9 个源域维护有标注与伪标注两组类别记忆，动量 $\mu_L$ 和 $\mu_P$ 分别为 0.95 和 0.995。样本--类别核心在特征分辨率下至少包含 4 个像素，超过 256 个像素时进行均匀空间抽样；伪标注核心写入要求 $w\geq0.8$。每类可靠查询先均匀抽取至多 128 个像素，伪标注查询要求 $w\geq0.6$，再保留正锚余弦距离最大的 25\%。Stage II 的 0--5,000 次迭代只更新有标注记忆，5,000--15,000 次将原型损失线性升至设定权重；15,000 次起开放伪标注记忆，其融合比例在 10,000 次内由 0 增至 0.2。对比温度为 0.3，对比、带容差同类对齐和困难负样本项的最终权重分别为 0.05、0.05 和 0.02；对齐容差和叶--茎排序间隔分别为 0.05 和 0.2。每个困难查询读取原型时排除自身域。\BAZR 从步长为 128 的 $256\times256$ 候选窗口中选择 $K=4$ 个区域，将其放大到 $512\times512$，NMS 阈值设为 0.3；熵、异常端点、尺度分歧和茎秆概率的路由权重依次为 1.0、2.0、1.0 和 0.5。
